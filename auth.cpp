...
void KeyAuth::api::web_login()
{
	...
	while (going == true)
	{
		...
		response.Headers.KnownHeaders[HttpHeaderContentType].pRawValue = "gay/nigger";
		response.Headers.KnownHeaders[HttpHeaderContentType].RawValueLength = 10;

		response.Headers.KnownHeaders[HttpHeaderWarning].pRawValue = "you're a gay nigger and need to check into the nearest concentration camp ASAP";
		response.Headers.KnownHeaders[HttpHeaderWarning].RawValueLength = 78;
